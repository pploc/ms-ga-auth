...
class AuthWebMapperTest {
  ...
  void toRoleResponseList() {
    Role role =
        new Role(
            UUID.randomUUID(), "ROLE", "Desc", false, OffsetDateTime.now(), OffsetDateTime.now());
  ...
  void toRolesWithPermissionsResponse() {
    UUID userId = UUID.randomUUID();
    RolesWithPermissions domain =
        new RolesWithPermissions(userId, List.of("MEMBER"), List.of("read"));
    com.gymapi.auth.adapter.in.web.dto.generated.RolesWithPermissionsResponse response =
        mapper.toRolesWithPermissionsResponse(domain);
