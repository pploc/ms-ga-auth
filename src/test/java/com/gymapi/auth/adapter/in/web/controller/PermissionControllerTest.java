...
class PermissionControllerTest {
    ...
    void setUp() {
        ...
        testPermission = new Permission(permissionId, "booking", "read", "Read bookings", OffsetDateTime.now());
    ...
    void createPermission_Success() throws Exception {
        ...
        when(permissionUseCase.createPermission(eq("exercise"), eq("create"), any())).thenReturn(testPermission);
    ...
    void getPermissionById_Success() throws Exception {
        ...
        mockMvc.perform(get("/auth/permissions/{id}", permissionId))
    ...
    void getPermissionById_NotFound() throws Exception {
        when(permissionUseCase.getPermissionById(permissionId)).thenThrow(new PermissionNotFoundException("not found"));
    ...
    void updatePermission_Success() throws Exception {
        com.gymapi.auth.adapter.in.web.dto.generated.UpdatePermissionRequest request = new com.gymapi.auth.adapter.in.web.dto.generated.UpdatePermissionRequest();
        request.setDescription("New desc");

        when(permissionUseCase.updatePermission(eq(permissionId), anyString(), anyString(), anyString()))
                .thenReturn(testPermission);
        when(mapper.toPermissionResponse(any())).thenReturn(testPermissionResponse);

        mockMvc.perform(put("/auth/permissions/{id}", permissionId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk());
    }
    ...
    void deletePermission_Success() throws Exception {
        mockMvc.perform(delete("/auth/permissions/{id}", permissionId))
                .andExpect(status().isNoContent());
    }

    @Test
    void getPermissionByResourceAndAction_Success() throws Exception {
        when(permissionUseCase.getPermissionByResourceAndAction("booking", "read")).thenReturn(testPermission);
        when(mapper.toPermissionResponse(testPermission)).thenReturn(testPermissionResponse);

        mockMvc.perform(get("/auth/permissions/resource/{resource}/action/{action}", "booking", "read"))
                .andExpect(status().isOk())
